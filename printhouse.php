...
$pdf->Ln(1);
$pdf->SetFont('', 'B', 12);
$pdf->Cell(0, 2, 'Tenant History', 0, 1, 'L');
$pdf->SetFont('', 'B', 9);
// Table 4
...
$result = mysqli_query($conn, "SELECT * FROM former_tenant where house = '$tnt' ");

$html = '<table cellspacing="" width="100%" style="padding: 3px; width: 100%; ">';
$html .= '<tr>
            <td style="border: 1px solid black; font-weight: bold; font-size: 13px;">Tenant</td>
            <td style="border: 1px solid black; font-weight: bold; font-size: 13px;">Exit Date</td>
            <td style="border: 1px solid black; font-weight: bold; font-size: 13px;">Reason</td>
         </tr>';

// Loop through the results and add rows to the HTML table
while ($row = mysqli_fetch_assoc($result)) {
    $html .= '<tr>
                <td style="border: 1px solid black;">' . htmlspecialchars($row['t_id']) . '</td>
                <td style="border: 1px solid black;">' . htmlspecialchars($row['to']) . '</td>
                <td style="border: 1px solid black;">' . htmlspecialchars($row['reason']) . '</td>
             </tr>';
}
...
$pdf->Ln(1);
$pdf->SetFont('', 'B', 12);
$pdf->Cell(0, 8, 'Fault History', 0, 1, 'L');
$pdf->SetFont('', 'B', 9);


// Assuming you have a MySQLi connection named $conn
$result = mysqli_query($conn, "SELECT * FROM faults where house = '$tnt'");

$pdf->SetFont('', '', 9);
$html = '<br> <br><table cellspacing="" width="100%" style="padding: 3px; width: 100%;">';
$html .= '<tr>
            <td style="border: 1px solid black; font-weight: bold; font-size: 13px;">Fault</td>
            <td style="border: 1px solid black; font-weight: bold; font-size: 13px;">Report Date</td>
            <td style="border: 1px solid black; font-weight: bold; font-size: 13px;">Status</td>
            <td style="border: 1px solid black; font-weight: bold; font-size: 13px;">Fix Date</td>
            <td style="border: 1px solid black; font-weight: bold; font-size: 13px;">Reporter</td>
         </tr>';

// Loop through the results and add rows to the HTML table
while ($row = mysqli_fetch_assoc($result)) {
    $html .= '<tr>
                <td style="border: 1px solid black;">' . htmlspecialchars($row['fault']) . '</td>
                <td style="border: 1px solid black;">' . htmlspecialchars($row['report_date']) . '</td>
                <td style="border: 1px solid black;">' . htmlspecialchars($row['status']) . '</td>
                <td style="border: 1px solid black;">' . htmlspecialchars($row['fix_date']) . '</td>
                <td style="border: 1px solid black;">' . htmlspecialchars($row['reporter']) . '</td>
             </tr>';
}

$html .= '</table>';

// Output or use $html as needed for your PDF generation
